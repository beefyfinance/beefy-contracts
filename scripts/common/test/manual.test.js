...
const GAS_CONFIG = { gasPrice: ethers.utils.parseUnits("6", "gwei"), gasLimit: 1e6 };
...
const main = async () => {
  try {
    ...
    let write = writer({ dirname: `${__dirname}/logs`, filename: `${hardhat.network.name}-manual-test-${symbol}` });
    ...
    console.log(
      "\tTransfering Strategy ownership to => ",
      addressBook[CHAIN_NAME].platforms.beefyfinance.strategyOwner
    );
    if (ethers.utils.isAddress(addressBook[CHAIN_NAME].platforms.beefyfinance.strategyOwner)) {
      let tx9b = await strategy.transferOwnership(
        addressBook[CHAIN_NAME].platforms.beefyfinance.strategyOwner,
        GAS_CONFIG
      );
