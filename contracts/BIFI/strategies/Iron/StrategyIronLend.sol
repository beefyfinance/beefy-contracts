...
contract StrategyIronLend is StratManager, FeeManager {
    ...
    constructor(
        ...
        comptroller = IVToken(iToken).ironController();
    ...
    function harvest() public whenNotPaused {
        ...
        if (IComptroller(comptroller).pendingIronControllerImplementation() == address(0)) {
            IComptroller(comptroller).claimReward(address(this));
            chargeFees();
            swapRewards();
            deposit();
        } else {
     ...
     * and then deposits again after the withdraw to make sure it maintains the desired ratio.
    ...
    function userReserves() public view returns (uint256,uint256) {
        (, uint256 iTokenBal, uint256 borrowBal, uint256 exchangeRate) = IVToken(iToken).getAccountSnapshot(address(this));
        uint256 supplyBal = iTokenBal.mul(exchangeRate).div(10**18);
    ...
    // calculate balance of underlying 'want' held by the pool.
