...
contract StrategyNyanswopLP is Ownable, Pausable {
     ...
     * {nya} - Token generated by staking our funds. In this case it's the {nya} token.
     ...
     * {unirouter} - NyanSwop unirouter
     * {stakingRewards} - StakingRewards contract. Stake Tokens, get rewards.
     */
    address constant public unirouter = address(0xc946764369623F560a5962D32c1D16D45F1BD6fa);
     ...
     * {nyaToWbnbRoute} - Route we take to get from {nya} into {wbnb}.
     * {wbnbToBifiRoute} - Route we take to get from {wbnb} into {bifi}.
     * {nyaToLp0Route} - Route we take to get from {nya} into {lpToken0}.
     * {nyaToLp1Route} - Route we take to get from {nya} into {lpToken1}.
     */
     ...
     */
    constructor(address _lpPair, address _vault, address _stakingRewards) public {
        lpPair = _lpPair;
        ...
        IERC20(nya).safeApprove(unirouter, uint(-1));
    ...
    function deposit() public whenNotPaused {
        ...
        if (pairBal > 0) {
            ...
            IERC20(lpPair).safeApprove(stakingRewards, pairBal);
    ...
    function chargeFees() internal {
        ...
        IUniswapRouter(unirouter).swapExactTokensForTokens(toWbnb, 0, nyaToWbnbRoute, address(this), now.add(600));
    ...
    /**
     * @dev Swaps {nya} for {lpToken0}, {lpToken1} & {wbnb} using ThugSwap.
     */
    function addLiquidity() internal { 
        uint256 nyaHalf = IERC20(nya).balanceOf(address(this)).div(2);

        if (lpToken0 != nya) {
            IUniswapRouter(unirouter).swapExactTokensForTokens(nyaHalf, 0, nyaToLp0Route, address(this), now.add(600));
        }

        if (lpToken1 != nya) {
            IUniswapRouter(unirouter).swapExactTokensForTokens(nyaHalf, 0, nyaToLp1Route, address(this), now.add(600));
        }
    ...
    /**
     * @dev It calculates how much {lpPair} the strategy has allocated in the StakingRewards
     */
    ...
    function pause() public onlyOwner {
        ...
        IERC20(nya).safeApprove(unirouter, 0);
