...
/**
 * @dev Implementation of a strategy to get yields from farming LP Pools in Typhoon.
 */
contract StrategyTyphoonLP is Ownable, Pausable, GasThrottler {
     ...
     * {output} - Token generated by staking our funds. In this case it's the TYPH token.
    ...
    address constant public lpPair = address(0xc9299FE3656d2d1BE9F5f05D046c33b88BB762cA);
    ...
    address constant public masterchef = address(0x2A687d070A8870c723486Bb295756E3786549Ae7);
    ...
    function deposit() public whenNotPaused {
        ...
        if (pairBal > 0) {
            ILPStaking(masterchef).deposit(pairBal);
        }
    }
    ...
    function withdraw(uint256 _amount) external {
        ...
        if (pairBal < _amount) {
            ILPStaking(masterchef).withdraw(_amount.sub(pairBal));
    ...
    function addLiquidity() internal {
        ...
        if (lpToken0 != output) {
            IUniswapRouter(unirouter).swapExactTokensForTokensSupportingFeeOnTransferTokens(outputHalf, 0, outputToLp0Route, address(this), now.add(600));
        }

        if (lpToken1 != output) {
            IUniswapRouter(unirouter).swapExactTokensForTokensSupportingFeeOnTransferTokens(outputHalf, 0, outputToLp1Route, address(this), now.add(600));
        }
