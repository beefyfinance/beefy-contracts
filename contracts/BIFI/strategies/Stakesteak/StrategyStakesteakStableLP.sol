...
import "../../interfaces/stakesteak/ISteakHouseAMMPair.sol";
import "../../interfaces/stakesteak/ISteakHouseV2.sol";
...
contract StrategyStakesteakStableLP is StratManager, FeeManager {
    ...
    constructor(
        ...
        // setup lp routing
        lpToken0 = ISteakHouseAMMPair(want).token0();
        ...
        lpToken1 = ISteakHouseAMMPair(want).token1();
    ...
    function deposit() public whenNotPaused {
        require(ISteakHouseV2(chef).depositFee() <= 5000, "! Deposit Fee too high");
        uint256 wantBal = IERC20(want).balanceOf(address(this));

        if (wantBal > 0) {
            ISteakHouseV2(chef).deposit(poolId, wantBal);
        }
    }
    ...
    function withdraw(uint256 _amount) external {
        ...
        if (wantBal < _amount) {
            ISteakHouseV2(chef).withdraw(poolId, _amount.sub(wantBal));
    ...
    function harvest() public whenNotPaused onlyEOA {
        ISteakHouseV2(chef).deposit(poolId, 0);
        chargeFees();
        addLiquidity();
        deposit();
    ...
    function addLiquidity() internal {
        ...
        if (lpToken0 != output && lp0Amount < lp1Amount) {
            IUniswapRouterETH(unirouter).swapExactTokensForTokens(outputAll, 0, outputToLp0Route, address(this), now);
        }

        if (lpToken1 != output && lp1Amount <= lp0Amount) {
            IUniswapRouterETH(unirouter).swapExactTokensForTokens(outputAll, 0, outputToLp1Route, address(this), now);
        }

        uint256 lp0Bal = IERC20(lpToken0).balanceOf(address(this));
        uint256 lp1Bal = IERC20(lpToken1).balanceOf(address(this));
        ISteakHouseAMMPair(want).add_liquidity(lp0Bal, lp1Bal, 1, address(this));
    }
    ...
    function retireStrat() external {
        ...
        ISteakHouseV2(chef).emergencyWithdraw(poolId);
