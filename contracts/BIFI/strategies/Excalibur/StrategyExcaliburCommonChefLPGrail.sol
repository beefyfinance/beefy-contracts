...
import "../../interfaces/excalibur/IGrailToken.sol";
...
contract StrategyExcaliburCommonChefLPGrail is StratManager, FeeManager, GasThrottler {
    ...
    address public grail = 0x1a8fd04b7eBEB82605829EEA91324D81b41d6dcf;
    ...
    function _harvest(address callFeeRecipient) internal whenNotPaused {
        ...
        uint256 grailBal = IERC20(grail).balanceOf(address(this));
        IGrailToken(grail).unwrap(grailBal);
    ...
    function callReward() public view returns (uint256) {
        uint256 grailBal = rewardsAvailable();
        uint256 outputBal = IGrailToken(grail).getExpectedUnwrappedTokenAmount(address(this),grailBal);
        uint256 nativeOut;
        if (outputBal > 0) {
