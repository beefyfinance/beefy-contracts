...
//Lending Strategy
contract StrategyBankerJoeNative is StratManager, FeeManager {
    ...
    function _harvest(address callFeeRecipient) internal whenNotPaused {
        if (IComptroller(comptroller).pendingImplementation() == address(0)) {
            ...
            if (outputBal > 0) {
                chargeFees(callFeeRecipient, beforeBal);
    ...
    // performance fees
    function chargeFees(address callFeeRecipient, uint256 beforeBal) internal {
        uint256 toNative = IERC20(output).balanceOf(address(this));
        IUniswapRouter(unirouter).swapExactTokensForTokens(toNative, 0, outputToNativeRoute, address(this), now);

        uint256 nativeBal = availableWant().sub(beforeBal).mul(45).div(1000);
