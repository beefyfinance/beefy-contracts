...
contract StrategySbdoLP is Ownable, Pausable {
     ...
     * {bdoToWbnbRoute} - Route we take to get from {sbdo} into {wbnb}.
     ...
     */
    address[] public bdoToWbnbRoute = [sbdo, wbnb];
    ...
    constructor(address _lpPair, uint8 _poolId, address _vault, address _strategist) public {
        ...
        if (lpToken0 == wbnb) {
            sbdoToLp0Route = [sbdo, wbnb];
        } else if (lpToken0 != sbdo) {
            sbdoToLp0Route = [sbdo, wbnb, lpToken0];
        }

        if (lpToken1 == wbnb) {
            sbdoToLp1Route = [sbdo, wbnb];
        } else if (lpToken1 != sbdo) {
            sbdoToLp1Route = [sbdo, wbnb, lpToken1];
        }
    ...
    function addLiquidity() internal {   
        uint256 bdoHalf = IERC20(sbdo).balanceOf(address(this)).div(2);
        
        if (lpToken0 != sbdo) {
            IPancakeRouter(unirouter).swapExactTokensForTokens(bdoHalf, 0, sbdoToLp0Route, address(this), now.add(600));
        }

        if (lpToken1 != sbdo) {
            IPancakeRouter(unirouter).swapExactTokensForTokens(bdoHalf, 0, sbdoToLp1Route, address(this), now.add(600));
        }
