...
import { AlgebraOracleLibrary, IAlgebraPool, IAlgebraPlugin } from "../../utils/AlgebraOracleLibrary.sol";
import { BeefyOracleHelper, IBeefyOracle, BeefyOracleErrors } from "./BeefyOracleHelper.sol";

/// @title Beefy Oracle for Algebra Integral
/// @notice On-chain oracle using Algebra
contract BeefyOracleAlgebraIntegral {
    ...
    function getPrice(bytes calldata _data) external returns (uint256 price, bool success) {
        ...
        for (uint i; i < pools.length; i++) {
            ticks[i] = getTwapTick(pools[i], uint32(twapPeriods[i]));
        }
    ...
    function getTwapTick(address pool, uint32 secondsAgo) public view returns (int24 arithmeticMeanTick) {
        address plugin = IAlgebraPool(pool).plugin();
        require(plugin != address(0), "Pool missing plugin");

        uint32[] memory secondsAgos = new uint32[](2);
        secondsAgos[0] = secondsAgo;
        secondsAgos[1] = 0;

        (int56[] memory tickCumulatives, ) = IAlgebraPlugin(plugin).getTimepoints(secondsAgos);
        int56 tickCumulativeDelta = tickCumulatives[1] - tickCumulatives[0];
        arithmeticMeanTick = int24(tickCumulativeDelta / int32(secondsAgo));

        if (tickCumulativeDelta < 0 && (tickCumulativeDelta % int32(secondsAgo) != 0)) {
            arithmeticMeanTick--;
        }
    }
